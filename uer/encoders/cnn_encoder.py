...
class GatedcnnEncoder(nn.Module):
    def __init__(self, args, encoder_conf):
        ...
        self.emb_size = args.emb_size[encoder_conf]
        self.hidden_size = args.hidden_size[encoder_conf]

        self.conv_1 = nn.Conv2d(1, args.hidden_size[encoder_conf], (args.kernel_size, args.emb_size[encoder_conf]))
        self.gate_1 = nn.Conv2d(1, args.hidden_size[encoder_conf], (args.kernel_size, args.emb_size[encoder_conf]))

        self.conv_b1 = nn.Parameter(torch.randn(1, args.hidden_size[encoder_conf], 1, 1))
        self.gate_b1 = nn.Parameter(torch.randn(1, args.hidden_size[encoder_conf], 1, 1))

        self.conv = nn.ModuleList(
            [
                nn.Conv2d(args.hidden_size[encoder_conf], args.hidden_size[encoder_conf], (args.kernel_size, 1))
                for _ in range(args.layers_num[encoder_conf] - 1)
            ]
        )
        ...
        self.conv_b = nn.ParameterList(
            nn.Parameter(torch.randn(1, args.hidden_size[encoder_conf], 1, 1))
            for _ in range(args.layers_num[encoder_conf] - 1)
        )
