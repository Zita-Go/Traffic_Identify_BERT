    ...
    def worker(self, proc_id, start, end):
        ...
        with open(self.corpus_path, mode="r", encoding="utf-8") as f:
            ...
            while True:
                ...
                # line为换行符时表示上一个流量对，document包含2个元素
                if not line.strip():
                    if len(document) >= 1:
                        docs_buffer.append(document)
                    document = []
                    # 当流量对的数量达到docs_buffer_size时，docs_buffer.size = [100000, 2, l]
                    if len(docs_buffer) == self.docs_buffer_size:
                        ...
                        docs_buffer = []
                    continue
                # 进行预处理并以空格分隔为token，然后再将token转化为id，sentence为列表
        ...
        while i < len(document):
            ...
            if i == len(document) - 1 or current_length >= target_seq_length:
                if current_chunk:
                    ...
                    # 样本结构为[CLS] + [tokens_a] + [SEP] + [tokens_b] + [SEP]
                    src = []
                    ...
                    if not self.dynamic_masking:
                        ...
                        # 分别为经过mask后的序列，被mask的位置和token，是否随机匹配（是否正样本），a段长度和总长度
